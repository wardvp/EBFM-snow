...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Fresh snow density
OUT.Dfreshsnow_T(IN.T>C.T0+2) = 50+1.7*17^(3/2);                            % SOURCE: Kampenhout et al. (2017)
...
while any(shift_tot>0)
    ...
    OUT.subSOIL(i_shift,2:nl) = subSOIL_old(i_shift,1:nl-1);  
    OUT.subSOIL(i_shift,1) = 0;
...
while any(shift_tot<0)
    ...
    shift(OUT.subSOIL(:,2)==1,1) = max(shift(OUT.subSOIL(:,2)==1,1),-OUT.subZ(OUT.subSOIL(:,2)==1,1));
    shift(OUT.subSOIL(:,1)==1,1) = 0;
    ...
    OUT.subSOIL(i_shift,1:nl-1) = subSOIL_old(i_shift,2:nl);

    OUT.subZ(i_shift,nl) = grid.max_subZ;
    OUT.subSOIL(i_shift,nl) = 1;
    OUT.subD(i_shift,nl) = C.soil_density;
...
kk = (0.138-1.01d-3.*OUT.subD+3.233d-6.*OUT.subD.^2) .* (OUT.subSOIL==0)... 
    + C.soil_Kfrozen .* (OUT.subSOIL==1 & OUT.subT<C.T0) ...
    + C.soil_Kthawed .* (OUT.subSOIL==1 & OUT.subT>=C.T0); 
 
% effective heat capacity                                                   % SOURCE: Yen et al. (1983) [snow], Westermann et al. (2011) [soil] 
c_eff = OUT.subD .* (152.2+7.122.*OUT.subT) .* (OUT.subSOIL==0) ...
    + (C.soil_Cfrozen + C.Lm.*1d3.*(C.soil_THwmax-C.soil_THwmin) ...
    .*C.soil_delta./(OUT.subT-C.T0-C.soil_delta).^2) ...
    .* (OUT.subSOIL==1 & OUT.subT<C.T0) ...
    + C.soil_Cthawed .* (OUT.subSOIL==1 & OUT.subT>=C.T0);
...
OUT.subT(OUT.subT>C.T0 & OUT.subSOIL==0) = C.T0;
OUT.subCeff = c_eff;
OUT.subK = kk;
